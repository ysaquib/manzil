...
**Rubric** — view mode default: enabled criteria as cards with an options table (match label left, colored points text right; bonus and non-negotiable badges on the card header). Match labels omit assumed operators (`eq`/`bool`) and use `<`/`≤`/`>`/`≥` for inequalities. Owner edits via a single-page editor (all catalog criteria on one scrollable page with enable toggles, option deltas, dealbreakers, and gate controls); first-run hunts (no saved rubric) open directly in edit mode. Widgets rendered from `value_schema`. Read-only view for non-Owners (Phase 2).
