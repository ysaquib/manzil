...
| availability_date | availability | date (ISO; immediate-availability phrases extract as transient sentinel `available_now`, rewritten to the run date in EXTRACT — §20 2026-07-16) | — | pricing |
...
| 2026-07-16 | **`availability_date`: immediate-availability phrases → transient sentinel `available_now`, rewritten to the run date in EXTRACT.** Pages often say "Available Now" / "Now" / "Immediately" with no calendar date. The LLM identifies that fact as the exact sentinel `available_now` (catalog `extraction_hint` + floor-plans schema description); EXTRACT normalizes both the property-level criterion and each floor plan's `availability_date` to `ctx.today().isoformat()` before VERIFY/persist. Persisted and scored values remain ISO dates only — the sentinel never reaches scoring, Postgres, or VERIFY's past-date check. Bench labels use `"available_now"` for those pages; the harness matches the sentinel against the run date after normalize. Do not inject "today" into the EXTRACT prompt or invent a date in the hint — the model has no trustworthy clock. | Extraction should report page truth; calendar assignment is deterministic pipeline work. Hint-alone "use today's date" asks the model to invent a date it does not know. | §8.2, §10.3, §20 |
