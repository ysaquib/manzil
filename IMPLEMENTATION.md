...
| P2-4 | Invites: create (email via Supabase Auth invite — Mailpit locally — or copy-link token, 7 d expiry, revocable), accept via `privileged.py` (membership insert + next-unused color token from the Dusk & clay palette), `/invite/:token` page with post-login return | §9.1, §16 | second account joins via both paths; curator grant works; expired/revoked tokens refused |
...
*Status: implementation-prep draft, 2026-07-11. Finalize on Phase 3 entry after
Yusuf confirms P2-10; this does not declare Phase 2 exited.*
...
| D — lifecycle and UX | P3-11 checkpoints · P3-12 refresh Planner · P3-13 compare/mobile | Producing stages and persistence shapes are stable | P3-12 follows the stages whose TTL/hash inputs it plans; P3-13 can begin once comparison data shapes stabilize |
