...
*Status: Phase 0 table settled — tail (P0-11..14) still open, blocking nothing · Phase 1 **complete, exited 2026-07-10** · **Phase 2 table revised on entry** (2026-07-10, per `.claude/plans/phase-2-collaboration.md`) · Phase 3 table provisional (revise on entry) · ⚠ items conditional on named gates.*
...
| P2-9 | Curator end-to-end: overrides/fees/pins/checkpoints on others' listings allowed; job management and rubric/settings/members on others' denied | §4.2 | verified by P2-3 suite + manual UI pass |
| P2-10 | Exit: partner active in the real hunt; revise Phase 3 table on entry (with the P0-14 census/model verdicts in hand — they scope P3-14) | §19 | both members using it in anger |
...
*Status: implementation-prep draft, 2026-07-11; elaborated by
`.claude/plans/phase-3-agent-system.md` (2026-07-11), which finalizes this table
at entry. Wave A (P3-2, P3-3) and Wave B's P3-4 (DEDUPE + `resolve_dedupe` +
...
|---|---|---|---|
| Phase 2 human exit | Formal Phase 3 entry | Pending: real partner workflow, Curator UI pass, two-browser Realtime <2 s, CI confirmation | Yusuf confirms P2-10; then update DESIGN §19/AGENTS status and finalize this table |
| P0-14 census verdict | P3-14; fetch-tier choice in P3-15 | **Open.** Census CSV exists, but the keep/provider/Apify verdict is not in DESIGN §20 | Finish P0-11..14; retain, rescope, or delete P3-14 from evidence |
| P0-14 model-pin verdict | Default models for new P3 LLM stages | **Open.** One local label/report; current Haiku/Sonnet pins remain baseline, not bench-settled | Complete labels + model comparison before changing any default pin |
