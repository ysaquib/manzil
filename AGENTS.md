...
**Phase 0 (DESIGN.md §19) — closing out.** The pipeline spine (P0-1..P0-10) is done: `shared/` engine + catalog, migrations (global tables only), fetch tiers + outcome classifier + tier-3 free-plan stopgap, VALIDATE_URL → FETCH → VALIDATE → EXTRACT → VERIFY → SCORE, CLI `ingest <url>`, Langfuse wiring. Remaining, and safe to run alongside Phase 2: P0-11 (bench labeling — human-only, in progress at the trimmed 10-listing scope, DESIGN §20 2026-07-17), P0-12/13 (eval harness + model bench — code landed, real runs pending; the sweep covers the full `MODEL_PRICES` candidate pool incl. the Gemini 3.x Flash tiers), and P0-14's model-pin entry. **P0-14's census half was ruled early 2026-07-17 (DESIGN §20): keep tier 3, Bright Data, free plan; Apify stays deferred — P3-14 is retained.** Nothing in Phase 2 depends on these — the model pin only affects `llm/config.py`.
...
**Phase 3 (DESIGN.md §19) — formally entered 2026-07-18; §7 table finalized at entry (IMPLEMENTATION 2.0.56, DESIGN §20).** Waves A/B plus P3-7a ran ahead of entry (Yusuf-directed; Phase-0-tail precedent). Landed: P3-2 (planner v1 + manifest-driven runner + plan/cost persistence), P3-3 (tool registry with seam-enforced per-stage allow-lists, `call_agent` loop, `fetch_page`, Maps tools + geocode forever-cache), P3-4 (DEDUPE between EXTRACT and VERIFY — zero tools, geocode as a plain call; `resolve_dedupe` checkpoint defaulting `keep_separate`; merge absorbs the submit-time placeholder Property at terminal projection; `manzil split-property` reverses a wrong merge and enqueues rescores — DESIGN §20 2026-07-12, IMPLEMENTATION §9 2.0.41), and **P3-7a** (IMAGE_FETCH discovery/download/normalized WebP/private Storage metadata, content-hash skip, manifest-aware runner skip, hash-only `call_vision` record/replay — DESIGN v3.3 / IMPLEMENTATION 2.0.46). P3-7b remains fail-closed until Yusuf approves the versioned reference set and bench; follow `docs/p3-7-vision-guide.md`. **P3-8 landed 2026-07-18 ◐** (IMPLEMENTATION 2.0.57–2.0.58, DESIGN §20 2026-07-18): ENRICH between VERIFY and SCORE — grocery proximity per `proximity_mode`, Places rating + review synthesis (`enrich_reviews` prompt), and `location_safety` rescoped to an **override-first A+..F placeholder** (no synthesis; the assessor is the deferred P3-17 safety module — ARCGIS/police-report based, SE Michigan first, possibly standalone); proximity-mode flip → `refresh` job (`scope: enrich`), zero LLM spend; first `catalog_sync` migration; ◐ pending one live Detroit-metro ENRICH run. **P3-9 landed 2026-07-18 ◐** (IMPLEMENTATION 2.0.59, DESIGN §20): full §9.5 composition (mandatory-fees/heating EXTRACT blocks — optional-with-default, no prompt bump; graduated unknown rule; fee-suppresses-estimate; occupants now a scoring key) + the utility-baselines job on the new **scheduler-tick scaffold** (NOT a jobs row; advisory-lock-guarded; opt-in seam wired only by the api worker entry — P3-11/P3-12 extend it); baselines pass is search-less until P3-5; display composition on `hunt_listings.all_in_components`; ◐ pending the first live baselines pass + the owed bench re-run. Plan: `.claude/plans/phase-3-agent-system.md`. Also landed 2026-07-12: the fetcher-layer SSRF guard (DESIGN §20; IMPLEMENTATION §9 2.0.40) — the HTTP path is closed with tier-1 IP-pinning. The Google Maps API key is configured (2026-07-17), clearing the Phase 3 credential gate. **Both pre-live SSRF smoke checks passed 2026-07-17** (DESIGN/IMPLEMENTATION §20; IMPLEMENTATION 2.0.53): tier-1's pin cert path is confirmed, and the tier-2 check surfaced that Playwright doesn't re-invoke the route handler on server 3xx redirects — hardened so the post-load backstop screens the whole `redirected_from` chain, not just `page.url`. The residual tier-2 DNS-rebinding window stays the accepted best-effort. P3-5/P3-10 are cleared on the SSRF gate. **Next: Wave B's P3-5 (DISCOVER)** — search provider is decision-with-default (plan §2.2: Anthropic server-side `web_search` via OpenRouter; verify passthrough at implementation). P3-14 is scoped by the census verdict: Bright Data free plan, per-domain registry pins for the five census-named hostile domains only. New P3 LLM stages ride existing Haiku/Sonnet pins until the P0-14 model-pin verdict.
