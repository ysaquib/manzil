...
**Phase 3 (DESIGN.md §19) — formally entered 2026-07-18.** Landed: P3-2 planner/manifest runner, P3-3 tool registry/Maps, P3-4 DEDUPE/reversal, **P3-5 DISCOVER** (2026-07-21, DESIGN v3.8 / IMPLEMENTATION 2.0.70: OpenRouter native search, exact same-Property candidate filtering, official link-only persistence, tier/family slate, editable Source Policy, reasoned single-source badge), **P3-SC2 scoped Extraction foundation** (2026-07-21, DESIGN v3.9 / IMPLEMENTATION 2.0.71: append-only candidate/resolved facts, Source-local Floor Plan identity, centralized current views/effective resolver, scoped Overrides, authoritative refresh and same-Property guards), **P3-SC3 Property Catalog/set-valued Rubric path** (2026-07-22, DESIGN v3.10 / IMPLEMENTATION 2.0.72: 13-Criterion Property tranche, strict `contains_any/all`, persisted Floor Plan unit types, separate Property/plan presentation, guarded versioned dev Rubric), **P3-SC4 engineering ◐** (2026-07-27, DESIGN v3.15 / IMPLEMENTATION 2.0.78: sparse exact/all/select/unspecified claims for the existing unit tranche + heating, deterministic effective-presence/Gate behavior, scoped label/harness/audit tooling; human canonical-ten labels and current-pin zero-wrong-exact baseline remain), P3-7a image substrate, and **P3-19 map surfaces** (2026-07-26, DESIGN v3.14 / IMPLEMENTATION 2.0.77: drawer Location card + `/h/:huntId/map`, one band-sliced pin per Listing, unit-group picker, Overview filter state lifted to a shared provider; needs the separate browser key `VITE_GOOGLE_MAPS_API_KEY`). P3-8 and P3-9 are landed ◐ with their live/bench follow-ups recorded in IMPLEMENTATION. P3-7b remains fail-closed pending the approved reference set. The SSRF gate and Google Maps credential gate are cleared. P3-14 remains scoped to Bright Data's free plan and the five census-named hostile domains. The P0-14 pin moves only EXTRACT/VERIFY to `gemini-3-flash-preview`; unbenched stages keep their Anthropic tier. Plan: `.claude/plans/phase-3-agent-system.md`. **Next: finish P3-SC4's human label/baseline tail**; P3-6 remains blocked until that acceptance gate passes.
