...
| `MANZIL_TIER3_FALLBACK_PROVIDER`         | Optional distinct configured provider; decode failures only | no |
...
| Build identity             | Generated from root `VERSION` + deploy Git SHA; do not set manually |


Vite values are compiled at build time — changing one requires **Save, rebuild, and deploy**.
`VITE_APP_VERSION`, `VITE_RELEASE_VERSION`, and `VITE_BUILD_SHA` are compile-time
constants generated by `vite.config.ts`; the deploy's `RENDER_GIT_COMMIT` wins,
then CI's `GITHUB_SHA`, with `dev` only as the local fallback.
