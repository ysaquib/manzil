...
def corpus_pages(slugs: list[str] | None = None, corpus_dir: Path = CORPUS_DIR) -> list[Path]:
    ...
    return sorted(
        page_dir
        for page_dir in corpus_dir.iterdir()
        if (page_dir / "raw.html").exists() and (not slugs or page_dir.name in slugs)
    )
...
def regenerate_cleaned(slugs: list[str] | None = None) -> list[tuple[str, int, int]]:
