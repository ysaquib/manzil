...
def test_canton_without_locality_uses_postal_city() -> None:
    components = _components(
        country=[("United States", "US")],
        administrative_area_level_1=[("Michigan", "MI")],
        administrative_area_level_2=[("Wayne County", "Wayne")],
        administrative_area_level_3=[("Canton Township", "Canton")],
    )
    locality = parse_us_locality(components, "120 Main St, Canton, MI 48187")
    assert locality.city == "Canton"
...
def test_full_state_name_and_zip_plus_four() -> None:
    ...
    locality = parse_us_locality(
        components,
        "120 Main St, Canton, Michigan 48187-1234, USA",
    )
    assert locality.city == "Canton"
    assert locality.state == "MI"
...
def test_google_locality_wins_over_conflicting_address() -> None:
    components = _components(
        country=[("United States", "US")],
        locality=[("Ypsilanti", "Ypsilanti")],
        administrative_area_level_1=[("Michigan", "MI")],
    )
    locality = parse_us_locality(components, "120 Main St, Canton, MI 48187")
    assert locality.city == "Ypsilanti"
    assert locality.state == "MI"
...
def test_invalid_state_token_is_rejected() -> None:
    components = _components(country=[("United States", "US")])
    locality = parse_us_locality(components, "120 Main St, Canton, ZZ 48187")
    assert locality.city is None
...
def test_component_order_is_immaterial() -> None:
    first = _components(
        country=[("United States", "US")],
        locality=[("Canton", "Canton")],
        administrative_area_level_1=[("Michigan", "MI")],
        administrative_area_level_2=[("Wayne County", "Wayne")],
    )
    second = list(reversed(first))
    address = "120 Main St, Canton, MI 48187"
