...
async def test_enrich_refresh_rederives_grocery_and_rescores() -> None:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            resolution_origin = await conn.fetchval(
                """
            ...
            # The current row is the resolved fact. Its job-scoped origin links
            # to the stable Maps producer retained on the candidate row.
            assert resolution_origin.startswith("resolution:")
            candidate_origin = await conn.fetchval(
                """
            ...
            assert candidate_origin == "google_maps:grocery"
