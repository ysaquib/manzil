...
def test_runs_extract_then_verify_without_a_label(tmp_path: Path) -> None:
    ...
    assert result.floor_plans[0].beds == 2
...
def test_surfaces_verify_flags_without_losing_audit_result(tmp_path: Path) -> None:
    ...
    assert next(c for c in result.source_claims if c.criterion_key == "pool").value == "outdoor"
    assert any(flag.criterion_key == "pool" for flag in result.verify_flags)
    assert result.checkpoint is None
